function begin() { // Working
	//stat();
	//setTimeout(stat, 0);
	document.getElementById("clev").style.display = "none";
	document.getElementById("reclev").style.display = "inline";
	clevulate();
}
...
function clevulate() {
	...
	for (i = 0; i < clevWords.length; i++) {
		console.log('Clev Iteration' + clevWords[i][0] + " " + i);
		if (clevWords.length > 0 && paragraph.indexOf(clevWords[i][0]) > -1) { // <-- idk what this does lol
			console.log('True Clev Iteration' + clevWords[i][0]);
			searchResults.push(clevWords[i][0]);
			let l = clevWords[i][1].length;
			let r = random(0,l-1);
			searchReplacements.push(clevWords[i][1][r]);
			suggestTab(clevWords[i][0], clevWords[i][1][r], i);
			/*
	...
	if (searchResults.length == 0) { //Handle No clevWords
...
function recalculate() {
	...
	for (i = 0; i < clevWords.length; i++) {
		if (clevWords.length > 0 && paragraph.indexOf(clevWords[i][0]) > -1) {
			recalculatedResults.push(clevWords[i][0]);
		}
	}
